...
class Trainer:
    def __init__(self, args):
        ...
        self.model.fc = nn.Linear(self.model.fc.in_features, self.class_num).to(self.device)
    ...
    def _train_loop(self, epoch, criterion):
        ...
        for step, (inputs, labels, _) in enumerate(self.trn_loader):
            ...
            if self.loss_mode == 'ce' :
                ...
                loss = criterion(torch.squeeze(output),labels,self.class_num,window_size=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--norm-type", default= 0, type=int, choices=[0, 1, 2],
    ...
    parser.add_argument(
        "--batch-size", default=8,type=int, help="Batch size"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--loss_mode", type=str, default='soft_ce',help="ce,soft_ce"
    )
    parser.add_argument(
        "--class_num", type=int, default=10 ,help="last output node num"
    )
